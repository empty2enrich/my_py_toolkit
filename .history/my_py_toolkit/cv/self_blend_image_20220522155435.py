...
def gen_img(img, landmarks, bbox, phase, image_size):
    landmarks = reorder_landmark(landmarks)
    if phase == 'train' and np.random.rand() < 0.5:
        img, _, landmarks, bbox = hflip(img, None, landmarks, bbox)
    img, landmarks, bbox, _ = crop_face(img, landmarks, bbox, margin=True,crop_by_bbox=False)
    img_r, img_f, mask_f = self_blending(img.copy(),landmarks.copy())
    if phase == 'train':
        transformed=transforms(image=img_f.astype('uint8'),image1=img_r.astype('uint8'))
        img_f=transformed['image']
        img_r=transformed['image1']
    (img_f, _, __, ___,
     y0_new, y1_new, x0_new, x1_new) = crop_face(img_f,landmarks,bbox,margin=False,
                                                        crop_by_bbox=True,abs_coord=True,phase=phase)			
