...
def handle_record(record, key):
    ...
    for k,v in mapping.items():
        if k not in mapping_reserve:
            mapping_reserve[k] = [v]
        else:
            mapping_reserve[k].append(v)
    for k in mapping_reserve.get(key, []):
        if k in record:
            return record[k]
    return ''
...
def save_data(datas, data_pah, save_dir, scale=1.5):
    os.makedirs(save_dir, exist_ok=True)
    fn = get_file_name(data_pah)
    for key, (empty, not_empty) in datas.items():
        fn_k = fn[:fn.rfind('.')] + f'_{key}.jsonl'
        cur_path = os.path.join(save_dir, fn_k)
        with open(cur_path, 'w', encoding='utf-8') as writer:
            ids = min(len(empty), len(not_empty)) * scale
            ids = int(ids)
            random.shuffle(empty)
            random.shuffle(not_empty)
            for v in empty[:ids] + not_empty[:ids]:
                writer.write(v)
...
def main():
    ...
    #     writers = get_wirter(keys, data_path, './finetune_v3')
                ...
                try:
                    ...
                    for i, prompt in enumerate(prompts):
                        for record in medical_records:
                            k, v = keys[i], handle_record(record, keys[i])
                            print(k, v)
                            cur_key_record = {keys[i]: handle_record(record, keys[i])}
                            record_str = json.dumps(cur_key_record, ensure_ascii=False)
                            cur_str = json.dumps({'input': prompt, 'target':record_str}, ensure_ascii=False) + '\n'
                            # for k,v in record.items():
                            if k not in datas_all:
                                datas_all[k] = [[], []]
                            if v in empty_mapping.get(k, []):
                                datas_all[k][0].append(cur_str)
                            else:
                                datas_all[k][1].append(cur_str)
                    ...
                    cts += 1
                    print(f'error: {cts} {line}, {traceback.format_exc()}')
                    print(cts)
                    line = r.readline()

    save_data(datas_all, data_path, save_dir='./finetune_v3', scale=1.5)
    save_data(datas_all, data_path, save_dir='./finetune_v4', scale=5)
